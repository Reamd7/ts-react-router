...
describe("<Route render>", () => {
  ...
  it("receives { match, location, history } props", () => {
    ...
    // expect(actual.history).toBe(history); // History Must False 被代理后的history 不能通过测试
...
describe("A <Route exact={true}>", () => {
  ...
  it("renders when the URL has trailing slash", () => {
    ...
    // expect(node.innerHTML).toContain(TEXT); // FIX TEST BUG
    /**
     * 据本人理解，Route 定义匹配路径path为/somepath/ 需要 测试中的 location.path"/somepath" 完整匹配到 path 才成功
     * 即 /somepath/ in "/somepath" 
     */
    expect(node.innerHTML).not.toContain(TEXT);
  });
});

describe("A <Route exact={true} strict={true}>", () => {
...
describe("A <Route location>", () => {
  ...
  describe("children", () => {
    ...
    it("continues to use parent's prop location after navigation", () => {   
      ...
      // expect(node.innerHTML).toContain(TEXT); 
      /**
       * FIX 值得商榷的test ,该test 的本质是测试Route 的多层相互独立的context
       * 是否需要在看看其他类库，特在此标注。
       */
