...
describe("A <Switch>", () => {
  ...
  it("handles subsequent redirects", () => {
    ...
    ReactDOM.render(
      <MemoryRouter initialEntries={["/one"]}>
        <Switch>
          <Redirect exact={true} from="/one" to="/two" />
          <Redirect exact={true} from="/two" to="/three" />
  ...
  it("warns when redirecting to same route, both strings", () => {
    ...
    expect((console.error as any/*Fix Hook*/).calls.count()).toBe(1);
    expect((console.error as any/*Fix Hook*/).calls.argsFor(0)[0]).toMatch(/Warning:.*"\/one"/);
  });
  ...
  it("warns when redirecting to same route, mixed types, string with query", () => {
    ...
    expect(node.innerHTML).not.toContain("done");
    expect((console.error as any/*Fix Hook*/).calls.count()).toBe(1);
    expect((console.error as any/*Fix Hook*/).calls.argsFor(0)[0]).toMatch(
      /Warning:.*"\/one\?utm=1"/
    );
  });
  ...
  it("does NOT warn when redirecting to same route with different `search`", () => {
    ...
    expect((console.error as any/*Fix Hook*/).calls.count()).toBe(0);
  });
  ...
  it("throws with no <Router>", () => {
    ...
    // expect((console.error as any/*Fix Hook*/).calls.count()).toBe(3); // Fix 未知错误
    expect((console.error as any/*Fix Hook*/).calls.count()).toBe(2);
    // expect((console.error as any/*Fix Hook*/).calls.argsFor(0)[0]).toContain(
...
describe("A <Switch location>", () => {
  it("can use a `location` prop instead of `router.location`", () => {
    ...
    ReactDOM.render(
      <MemoryRouter initialEntries={["/one"]}>
        <Switch location={{ pathname: "/two" } as H.Location}>
          <Route path="/one" render={() => <h1>one</h1>} />
          <Route path="/two" render={() => <h1>two</h1>} />
        </Switch>
      </MemoryRouter>,
  ...
  describe("children", () => {
    it("passes location prop to matched <Route>", () => {
      ...
      let propLocation;
      const RouteHoneytrap = (props:IRouteProps) => {
      ...
      ReactDOM.render(
        <MemoryRouter initialEntries={["/one"]}>
          <Switch location={switchLocation as H.Location}> {/* *TODO* */}
            <Route path="/one" render={() => <h1>one</h1>} />
