...
import { IRouteConfig, IRouteConfigComponentProps } from '../@type';
import * as H from 'history';

describe("renderRoutes", () => {
  let renderedRoutes:any[];
  let renderedExtraProps:any[];
  const Comp:React.ComponentType<IRouteConfigComponentProps<any> | any> = ({ route , ...extraProps }) => (
    renderedRoutes.push(route),
    renderedExtraProps.push(extraProps),
    renderRoutes(route!.routes) // TODO
  );
  ...
  it("renders pathless routes", () => {
    const routeToMatch:IRouteConfig = {
      component: Comp
    };
  ...
  describe("Switch usage", () => {
    ...
    it("does not remount a <Route>", () => {
      ...
      const App:React.ComponentType<IRouteConfigComponentProps<any>> = (props) =>{
        return renderRoutes(props.route!.routes); // TODO
      }
      ...
      let routerRef!:Router;  // TODO 必须使用实例化后的history 进行动态更新。
      ReactDOM.render(
        <Router ins={(ref)=>{
          routerRef = ref
        }} history={history}>{renderRoutes(routes)}</Router>,
        node
      );

      expect(mountCount).toBe(1);

      routerRef.history.push("/one");
      expect(mountCount).toBe(1);

      routerRef.history.push("/two");
      expect(mountCount).toBe(1);

      routerRef.history.push("/three");
    ...
    it("passes props to Switch", () => {
      ...
      ReactDOMServer.renderToString(
        <StaticRouter location="/two" context={{}}> 
          {renderRoutes(routes, {}, { location: { pathname: "/one" } as H.Location }) /* TODO */}
        </StaticRouter>
      );
  ...
  describe("routes with exact + strict", () => {
    ...
    it("renders the matched exact strict route but not its child routes if they do not match", () => {
      ...
      expect(renderedRoutes[1]).toEqual(routes[1].routes![1]); // TODO
    });
  });
  ...
  it("allows rendering a component using a function with render property", () => {
    const routes:any = [      
      {
        path: "/path",
        render: function(props:IRouteConfigComponentProps<any> | any){ // 不清楚如何处理这种类型
          return (<Comp {...props} />)
        }
      }
    ];
    ...
    ReactDOMServer.renderToString(
      <StaticRouter location="/path" context={{}}>
        {renderRoutes(routes as IRouteConfig[])}
      </StaticRouter>
    );
    expect(renderedRoutes.length).toEqual(1);
    expect(renderedRoutes[0]).toEqual(routes[0]);
